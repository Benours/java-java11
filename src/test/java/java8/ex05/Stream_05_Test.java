...
public class Stream_05_Test {
    ...
    private long iterateSum(long n) {
        return LongStream.iterate(0, i -> 1 + i).limit(n).sum();
    }
    ...
    private long parallelIterateSum(long n) {
        return LongStream.iterate(0L, i -> i + 1L).parallel().limit(n).sum();
    }
    ...
    public void monitor_imperativeSum_vs_iterateSum_vs_parallelIterateSum() {
        Logger.getAnonymousLogger().info("imperativeSum => " + /* TODO */ monitor(x -> imperativeSum(x), NB) + " ms");
        Logger.getAnonymousLogger().info("iterateSum => " + /* TODO */ monitor(x -> iterateSum(x), NB) + " ms");
        Logger.getAnonymousLogger().info("parallelIterateSum => " + /* TODO */ monitor(x -> parallelIterateSum(x), NB) + " ms");
    }
    ...
    public void monitor_imperativeSum_vs_iterateSum_vs_parallelIterateSum_vs_rangeSum_vs_rangeParallelSum() {
        ...
        Logger.getAnonymousLogger().info("iterateSum => " + /* TODO */ monitor(x -> iterateSum(x), NB) + " ms");
        Logger.getAnonymousLogger().info("parallelIterateSum => " + /* TODO */ monitor(x -> parallelIterateSum(x), NB) + " ms");
        Logger.getAnonymousLogger().info("rangeSum => " + /* TODO */ monitor(x -> rangeSum(x), NB) + " ms");
        Logger.getAnonymousLogger().info("rangeParallelSum => " + /* TODO */ monitor(x -> rangeParallelSum(x), NB) + " ms");
    }
