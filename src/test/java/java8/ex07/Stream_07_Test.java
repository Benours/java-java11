...
public class Stream_07_Test {
    ...
    Function<List<Pizza>, List<Pizza>> filterPizza = (pizzas) -> pizzas.stream().filter(x -> x.getPrice() >= 1000).collect(Collectors.toList());

    // TODO créer une fonction List<Pizza> -> List<Pizza>
    // TODO seules les pizzas ayant un prix >= 1000 sont conservées
    // TODO .parallel()
    Function<List<Pizza>, List<Pizza>> parallelFilterPizza = (pizzas) -> pizzas.stream().filter(x -> x.getPrice() >= 1000).parallel().collect(Collectors.toList());
